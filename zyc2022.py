...
class AnalysisTransform(tf.keras.Sequential):
  ...
  def __init__(self, latent_depth, input_dim):
    super().__init__()
...
class HyperAnalysisTransform(tf.keras.Sequential):
  ...
  def __init__(self, hyperprior_depth, input_dim):
    super().__init__()
...
# In my understanding, this part stays convolutional.
# Please note that in comparison to Zhu et al.
# - we keep LPR (lrp_transforms) -> no motivation given for removal.
# - for CC, LPR we use the approach described in Minnen et al., Appendix A.; 
#   i.e. the input to slice_1 is 320, input to slice_2 is 352 etc. rather than
#   32, 64, ... as described in Zhu et al. Appendix A, Figure 12, which splits
#   the latent_means + latent_scales equally across all slices.
...
class ZYC2022Model(tf.keras.Model):
  ...
  def __init__(self, patchsize, lmbda,
    ...
    super().__init__()
    self.input_dim = (patchsize, patchsize, 3)
    self.lmbda = lmbda
    ...
    self.analysis_transform = AnalysisTransform(latent_depth, self.input_dim)
    self.synthesis_transform = SynthesisTransform(self.input_dim)
    self.hyper_analysis_transform = HyperAnalysisTransform(hyperprior_depth, self.input_dim)
    self.hyper_synthesis_mean_transform = HyperSynthesisTransform(self.input_dim)
    self.hyper_synthesis_scale_transform = HyperSynthesisTransform(self.input_dim)
  ...
  def call(self, x, training):
    ...
    y = self.analysis_transform(x_flat)
    y_shape = (h//16, w//16, cfg_ga.embed_dim[-1])

    z = self.hyper_analysis_transform(y)
    z = tf.reshape(z, (-1, h//64, w//64, cfg_ha.embed_dim[-1]))
    ...
    z_hat = tf.reshape(z_hat, (-1, h//64 * w//64, cfg_ha.embed_dim[-1]))
    
    # Build the decoder (synthesis) half of the hierarchical autoencoder.
    latent_scales = self.hyper_synthesis_scale_transform(z_hat)
    latent_scales = tf.reshape(latent_scales, (-1, h//16, w//16, cfg_hs.embed_out_dim[-1]))
    latent_means = self.hyper_synthesis_mean_transform(z_hat)
    latent_means = tf.reshape(latent_means, (-1, h//16, w//16, cfg_hs.embed_out_dim[-1]))
    ...
    y = tf.reshape(y, (-1, h//16, w//16, cfg_ga.embed_dim[-1]))
    ...
    y_hat = tf.reshape(y_hat, (-1, h//16*w//16, cfg_ga.embed_dim[-1]))

    x_hat = self.synthesis_transform(y_hat)
    x_hat = tf.reshape(x_hat, (-1, h, w, c))
  ...
  def compress(self, x):
    ...
    # Build the encoder (analysis) half of the hierarchical autoencoder.
    y = self.analysis_transform(x_flat)
    y_temp = tf.reshape(y, (-1, h//16, w//16, cfg_ga.embed_dim[-1]))
    y_shape = tf.shape(y_temp)[1:-1]
